...
#
# whether to include the cobol backend
# currently does not work, because libgcobol needs 128bit float type
#
with_cobol=false
...
$with_cobol && { languages="$languages,cobol"; enable_libgcobol=--enable-libgcobol; }
ranlib=ranlib
STRIP=${STRIP-strip -p}
...
if $with_cobol; then
#
# Cobol support was added in gcc-15, so we need atleast that version as host-compiler
#
	case $GCC in
		*-[0-9]*-m32)
			m32=" -m32"
...
for INSTALL_DIR in "${PKG_DIR}" "${THISPKG_DIR}"; do
	...
	for tool in gcc gfortran gdc gccgo go gofmt gm2 \
	            gnat gnatbind gnatchop gnatclean gnatkr gnatlink gnatls gnatmake gnatname gnatprep gnatxref gcobol gcobc; do
...
#
	...
	for f in ${gccsubdir#/}/{cc1,cc1plus,cc1obj,cc1objplus,f951,d21,collect2,lto-wrapper,lto1,gnat1,gnat1why,gnat1sciln,go1,brig1,cc1gm2,cobol1,g++-mapper-server}${BUILD_EXEEXT} \
...
#
# create a separate archive for the cobol backend
#
if $with_cobol; then
	cobol=`find ${gccsubdir#/} -name "libgcobol*"`
	cobol="$cobol "`find ${gccsubdir#/} -name "cobol1*"`
	cobol="$cobol "${PREFIX#/}/bin/${TARGET}-gcob*
	${TAR} ${TAR_OPTS} -Jcf ${DIST_DIR}/${TARNAME}-cobol-${host}.tar.xz $cobol || exit 1
	rm -rf $cobol
fi
